...
int main() {
	std::cout << "Start Packet Analysis application." << std::endl;
	...
	auto div = [](double a, double b)->double {return b != 0 ? a / b : 0; };
	...
	packetLE.printClassifStatsTable(la.getTPGGraph().getEnvironment(), la.getTPGGraph().getRootVertices().at(0));
